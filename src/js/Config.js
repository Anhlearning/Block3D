class Config {
  constructor() {
    ...
    this.TeeBlockOut = 9;
    this.TangleLine = 10;
    this.defautlVersion = "10a25";
    this.version = this.defautlVersion;
    this.PlayableAdsGame = this.defaultGame;
    this.PlayableAdsType = this.defaultAds;

    this.linkAndroid =
      "https://play.google.com/store/apps/details?id=com.falcon.dm.water.cake.sort.puzzle";
    this.linkiOS =
      "https://apps.apple.com/us/app/cake-sort-color-puzzle-game/id6448392144";
    this.linkWebAds =
      "https://apps.apple.com/us/app/cake-sort-color-puzzle-game/id6448392144";

    if (
      this.PlayableAdsType === this.Adwords ||
      this.PlayableAdsType === this.Facebook ||
      this.PlayableAdsType === this.IronSource
    ) {
  ...
  openLinkApp() {
    var os = this.getOS();
    if (
      this.PlayableAdsType === this.Unity ||
      this.PlayableAdsType === this.Applovin
    ) {
      if (os === "Android") {
      ...
      if (os === "Android") {
        mraid.openStore(this.linkAndroid);
      } else if (os === "iOS") {
        mraid.openStore(this.linkiOS);
      } else {
        mraid.openStore(this.linkAndroid);
      }
