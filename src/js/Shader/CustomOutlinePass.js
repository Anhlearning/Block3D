import { Color, DepthTexture, MeshNormalMaterial, NearestFilter, RGBFormat, ShaderMaterial, UnsignedShortType, Vector2, Vector4, WebGLRenderTarget } from "three";
...
class CustomOutlinePass extends Pass {
	constructor(resolution, scene, camera) {
		...
		normalTarget.depthTexture = new DepthTexture();
		normalTarget.depthTexture.type = UnsignedShortType;
		...
		const depthTarget = new WebGLRenderTarget(this.resolution.x, this.resolution.y);
	...
	createOutlinePostProcessMaterial() {
		return new ShaderMaterial({
			uniforms: {
				...
				multiplierParameters: { value: new Vector4(1, 1, 1.5, 1) },
